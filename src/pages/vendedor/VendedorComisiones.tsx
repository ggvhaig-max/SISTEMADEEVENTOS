...
export function VendedorComisiones() {
  ...
  if (loading) return <div className="p-8 text-slate-900">Cargando mis comisiones...</div>;

  return (
    <div className="p-6">
      <div className="flex items-center gap-3 mb-8">
        <TrendingUp className="w-8 h-8 text-indigo-500" />
        <h1 className="text-2xl font-bold text-slate-900">Mis Comisiones y Liquidaciones</h1>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="bg-white/70 backdrop-blur-xl border border-white/50 shadow-sm p-6 rounded-xl border border-white/50 shadow-xl">
          <p className="text-slate-500 text-sm font-medium mb-1">Por Cobrar (Pendientes)</p>
          <p className="text-3xl font-bold text-orange-400">${totales.pendientes.toLocaleString()}</p>
        </div>
        <div className="bg-white/70 backdrop-blur-xl border border-white/50 shadow-sm p-6 rounded-xl border border-white/50 shadow-xl">
          <p className="text-slate-500 text-sm font-medium mb-1">Liquidado (Pagadas)</p>
          ...
          <table className="w-full text-left text-slate-600">
            <thead className="bg-slate-50/50 text-xs uppercase font-semibold text-slate-500">
              <tr>
                ...
                <th className="px-6 py-4 text-amber-500">Mi Comisión ($)</th>
            ...
            <tbody className="divide-y divide-slate-200/50/50">
              {comisiones.filter(c => filterEstado === 'todos' || c.estado === filterEstado).map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="px-6 py-4 font-bold text-amber-500">${c.monto_comision}</td>
