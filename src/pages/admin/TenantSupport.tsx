...
export const TenantSupport = () => {
  ...
  if (loading) return <div className="p-8 text-center text-gray-400">Cargando tickets...</div>;

  return (
    <div className="p-6 max-w-7xl mx-auto flex flex-col h-[calc(100vh-80px)]">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold font-outfit text-white">Centro de Soporte</h1>
          <p className="text-gray-400">Gestiona tus consultas y requerimientos técnicos</p>
        </div>
        <button
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        ...
        <div className={`w-1/3 bg-gray-800 rounded-xl border border-gray-700 flex flex-col overflow-hidden ${selectedTicket ? 'hidden md:flex' : 'w-full'}`}>
          <div className="p-4 border-b border-gray-700 bg-gray-800/50">
            <h2 className="font-semibold text-white">Mis Tickets ({tickets.length})</h2>
          </div>
            ...
            {tickets.length === 0 ? (
              ...
              tickets.map(ticket => (
                ...
                >
                  ...
                  <h3 className="text-white font-medium truncate">{ticket.asunto}</h3>
                  <div className="mt-2 flex items-center gap-2 text-xs text-gray-400 capitalize">
        ...
        {selectedTicket && (
          <div className="flex-1 bg-gray-800 rounded-xl border border-gray-700 flex flex-col overflow-hidden">
            <div className="p-4 border-b border-gray-700 bg-gray-800/50 flex justify-between items-center">
              <div>
                <h2 className="font-bold text-lg text-white">{selectedTicket.asunto}</h2>
                <div className="flex gap-3 text-sm mt-1">
                  ...
                  <span className="text-gray-400 flex items-center gap-1 capitalize">
                    {getPriorityIcon(selectedTicket.prioridad)} {selectedTicket.prioridad}
                  </span>
                </div>
              </div>
              <button onClick={() => setSelectedTicket(null)} className="md:hidden text-gray-400 hover:text-white">
                <X className="w-6 h-6" />
              </button>
            </div>
                ...
                return (
                  <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] rounded-xl p-4 ${
                      isMine ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-700 text-gray-100 rounded-bl-none'
                    }`}>
                      {!isMine && (
            ...
            {selectedTicket.estado !== 'cerrado' ? (
              <form onSubmit={handleReply} className="p-4 bg-gray-800 border-t border-gray-700 flex gap-2">
                ...
                <button
                  ...
                  className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white p-2 px-4 rounded-lg transition-colors flex items-center gap-2"
                >
            ...
            <form onSubmit={handleCreateTicket} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2.5 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Detalle del problema *</label>
                <textarea
                  required
                  value={primerMensaje}
                  ...
                  rows={4}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-4 py-2.5 text-white resize-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                />
              </div>
              <div className="pt-4 flex gap-3">
                ...
                <button
                  type="submit"
                  className="flex-1 px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20 font-medium"
                >
                  Crear Ticket
                </button>
              </div>
